...
const dur = Number(process.argv[2] || 45);
console.log('Testing duration', dur);
const dataUri = createSilentWavDataUri(dur);
const base64 = dataUri.split(',')[1];
const buf = Buffer.from(base64, 'base64');
const info = inspectWavBuffer(buf);
const computed = info.subchunk2Size / (info.sampleRate * (info.bitsPerSample / 8));
console.log('Computed duration', computed);
fs.writeFileSync(`./test-create-${dur}s.wav`, buf);
console.log('Wrote test-create file: test-create-' + dur + 's.wav, size:', buf.length);
