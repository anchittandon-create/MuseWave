...
/**
 * Ensure Rollup's `dist/native.js` loader exists. Some releases omit the file,
 * which causes Vite builds to fail when the optional native bindings aren't
 * bundled. We recreate the loader on install so CI environments stay stable.
 */
...
if (!existsSync(path.join(__dirname, '..', 'node_modules', 'rollup'))) {
  // Rollup not installed (e.g. optional dependency skipped) – nothing to do.
  process.exit(0);
}
...
const PLATFORM_PACKAGES = ${JSON.stringify(PLATFORM_PACKAGES, null, 2)};
