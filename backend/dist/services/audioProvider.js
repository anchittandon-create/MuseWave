...
function parsePlanMeta(metaString, jsonBuffer) {
    if (metaString) {
        try {
    ...
    if (jsonBuffer) {
        try {
            return { plan: JSON.parse(jsonBuffer.toString('utf-8')) };
        }
        catch (error) {
...
async function fileToAsset(filePath, mime, meta, durationSec) {
...
export class MockAudioProvider {
    async generate(planAssetId, seed) {
