...
export async function enhancePrompt(prompt) {
    ...
    if (!model)
        return `Enhanced: ${prompt} (mock response - no model)`;
    try {
        ...
        return `Enhanced: ${prompt} (API error: ${error.message})`;
    }
}
export async function suggestGenres(prompt) {
    if (!model)
    ...
    try {
        ...
        return text.split(',').map((g) => g.trim()).slice(0, 3);
    }
    catch (error) {
...
export async function generateMusicPlan(prompt, genre) {
    if (!model)
        return { plan: 'Mock plan', structure: ['intro', 'verse', 'chorus'] };
    try {
...
export async function auditMusicPlan(plan) {
    if (!model)
        return 'Mock audit: Plan looks good.';
    try {
...
export async function suggestArtists(prompt) {
    if (!model)
    ...
    try {
        ...
        const text = result.response.text();
        return text.split(',').map((a) => a.trim()).slice(0, 3);
    }
    catch (error) {
...
export async function suggestLanguages(prompt) {
    if (!model)
    ...
    try {
        ...
        const text = result.response.text();
        return text.split(',').map((l) => l.trim()).slice(0, 3);
    }
    catch (error) {
...
export async function enhanceLyrics(lyrics) {
    if (!model)
        return `Enhanced: ${lyrics} (mock response - no model)`;
    try {
        ...
        return `Enhanced: ${lyrics} (API error: ${error.message})`;
    }
}
