...
const DEFAULT_STRUCTURE = [
    { section: 'intro', lengthBars: 8 },
    { section: 'verse', lengthBars: 16 },
    { section: 'chorus', lengthBars: 16 },
    { section: 'bridge', lengthBars: 8 },
    { section: 'chorus', lengthBars: 16 },
    { section: 'outro', lengthBars: 8 },
];
...
function generateTitle(genre, prompt) {
    if (prompt) {
        return prompt.length > 60 ? `${prompt.slice(0, 57)}...` : prompt;
    }
    return `MuseWave ${genre?.toUpperCase() ?? 'TRACK'}`;
}
export function registerPlanWorker(queue) {
