...
export class MockAudioProvider {
    ...
    generateWithFfmpeg(plan, duration, outputPath) {
        return new Promise((resolve, reject) => {
            ...
            const filterParts = oscillators.map((osc, i) => `sine=frequency=${osc.freq}:duration=${osc.duration}[a${i}]`);
            ...
            args.push('-filter_complex', mixFilter, '-map', '[aout]', '-c:a', 'pcm_s16le', '-ar', '44100', '-ac', '2', '-t', duration.toString(), '-y', outputPath);
    ...
    buildOscillators(plan, duration) {
        const oscillators = [];
    ...
    generateHarmony(baseFreq, section) {
        const harmonies = {
