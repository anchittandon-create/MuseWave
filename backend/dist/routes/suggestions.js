import { enhancePrompt, suggestGenres, suggestArtists, suggestLanguages, suggestInstruments, enhanceLyrics, generateMusicPlan, auditMusicPlan, generateCreativeAssets, } from '../services/geminiService';
export const suggestionRoute = async (app) => {
    app.post('/enhance-prompt', async (request, reply) => {
        try {
            const payload = request.body?.context ?? request.body;
    ...
    app.post('/generate-music-plan', async (request, reply) => {
        try {
            ...
            const seed = typeof body?.creativitySeed === 'number' ? body.creativitySeed : Date.now() % 1_000_000;
