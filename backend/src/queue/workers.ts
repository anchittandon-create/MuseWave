...
export function registerWorkers(queue: Queue): void {
  // Plan generation worker
  queue.registerWorker('generate_plan', async (job: Job) => {
  ...
  queue.registerWorker('generate_audio', async (job: Job) => {
  ...
  queue.registerWorker('generate_vocals', async (job: Job) => {
    const { plan, duration, outputPath } = job.data;
  ...
  // Mixing worker
  queue.registerWorker('mix_audio', async (job: Job) => {
  ...
  queue.registerWorker('generate_video', async (job: Job) => {
  ...
  // Complete job worker
  queue.registerWorker('complete_job', async (job: Job) => {
