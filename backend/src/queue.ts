...
export class Queue {
  ...
  private workers = new Map<JobType, WorkerRuntime>();
  private pollTimer?: NodeJS.Timeout;
  private running = false;

  constructor(deps: { prisma: PrismaClient; logger: Logger }) {
  ...
  async enqueue(type: JobType, params: Record<string, unknown>, options: EnqueueOptions = {}): Promise<EnqueueResponse> {
    ...
    return { jobId: job.id, status: job.status as JobStatus, reused: false };
  }

  private computeDedupeKey(type: JobType, params: Record<string, unknown>, parentId?: string) {
    const payload = JSON.stringify({ type, params, parentId });
    return createHash('sha256').update(payload).digest('hex');
  }

  private wakeWorkers(type: JobType) {
    const runtime = this.workers.get(type);
    if (!runtime) return;
    this.kickWorker(runtime);
  }

  private kickWorker(runtime: WorkerRuntime) {
    if (!this.running) return;
  ...
  private async processNext(runtime: WorkerRuntime) {
    try {
  ...
  private async claimNextJob(type: JobType) {
    const now = new Date();
    for (;;) {
  ...
  private async executeJob(handler: JobHandler, job: QueueJob) {
    const log = this.logger.child({ jobId: job.id, jobType: job.type });
    log.info('Processing job');
    let result: JobHandlerResult | void;
    try {
  ...
  private async handleFailure(job: QueueJob, error: Error) {
  ...
  private materializeJob(record: any): QueueJob {
    ...
    const toStatus = (value: string | null | undefined): JobStatus => {
      const normalized = value ?? 'QUEUED';
      return (JOB_STATUS.includes(normalized as JobStatus) ? normalized : 'QUEUED') as JobStatus;
    };
    const toType = (value: string): JobType => {
