...
export class SafetyService {
  ...
  async checkContent(prompt: string): Promise<SafetyResult> {
    if (!this.genAI) {
      // Mock safety check
...
`;
    ...
    try {
      ...
      const safety = JSON.parse(jsonMatch[0]) as SafetyResult;
      return safety;
    } catch (error) {
  ...
  private mockSafetyCheck(prompt: string): SafetyResult {
