...
TASK:
Generate an ULTRA-DETAILED, immersive music production brief that reads like a professional studio session document. This should be a rich, multi-layered narrative that inspires immediate creative action.

STRUCTURE YOUR RESPONSE AS FOLLOWS:

**1. SONIC VISION (2-3 sentences):**
Paint a vivid picture of the track's overall atmosphere using sensory language. Describe the emotional journey, the physical feeling it creates, and the scene it evokes. Use metaphors and cinematic language.

**2. PRODUCTION BLUEPRINT (3-4 elements):**
Detail specific production techniques, sound design choices, and arrangement strategies:
- Synthesizer/instrument choices with timbral descriptions (e.g., "warm analog Juno pads", "detuned saw waves", "granular vocal textures")
- Rhythmic patterns and groove characteristics (e.g., "shuffled 16th hi-hats", "syncopated kick patterns", "breakbeat surgery")
- Spatial and temporal effects (e.g., "cathedral reverb on vocals", "sidechain compression from kick", "tape saturation warmth")
- Harmonic progression or modal characteristics (e.g., "Dorian mode explorations", "jazz-influenced seventh chords", "suspended chord tension")

**3. ARRANGEMENT NARRATIVE (2-3 sentences):**
Describe the track's structural journey from intro to outro. Include build-ups, breakdowns, drops, and how energy evolves. Mention specific moments or transitions that define the track.

**4. REFERENCE FUSION (if context provided):**
If genres/artists are mentioned, explicitly blend their signature elements into your description. For example:
- "Channeling ${context.artists?.[0] || 'Fred again..'}'s melodic sensibility with [specific technique]"
- "Fusing ${context.genres?.[0] || 'house'}'s groove with ${context.genres?.[1] || 'ambient'}'s textures"

**5. LYRICAL/THEMATIC INTEGRATION:**
${context.lyrics ? `Weave the lyrical theme "${context.lyrics.substring(0, 50)}..." into the sonic narrative, suggesting how vocals should be treated (pitch, effects, placement in mix).` : 'Focus on instrumental storytelling with melodic hooks that communicate emotion wordlessly.'}

**CRITICAL REQUIREMENTS:**
- Length: 100-150 words total
- Use specific music production terminology (avoid generic terms like "epic beat")
- Include at least 5 concrete production elements (synth types, effects, mixing techniques)
- Make it feel like a brief from a Grammy-winning producer
- Draw from 2025 trends: AI-enhanced synthesis, spatial audio, global fusion, hyperpop aesthetics

**STYLE EXAMPLES:**
❌ BAD: "Create an energetic electronic track with bass and synths."
✅ GOOD: "Craft a hypnotic progressive house odyssey where crystalline FM bells cascade over a rolling sub-808 bassline, building through filtered white noise risers into a euphoric drop featuring stacked vocal chops processed through Valhalla VintageVerb. The arrangement mirrors a sunrise—starting with minimal kick/hat patterns at 122 BPM in A minor, gradually layering detuned supersaw pads and jazz-influenced sus4 chord stabs before the 64-bar breakdown strips to just atmospheric field recordings and a whispered vocal mantra, then exploding back with full frequency spectrum and Haas-effect stereo widening."

Now generate the enhanced prompt. If "Current Prompt" is empty, invent an entirely original concept from scratch using the context.

Return ONLY a JSON object: {"prompt": "your ultra-detailed production brief here"}.`;
