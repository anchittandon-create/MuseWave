...
const schema = z.object({
  ...
  durationSec: z.number().min(30).max(600).default(90),
...
async function processGeneration(jobId: string, params: GenerateInput) {
  try {
    ...
    const bassNoteCount = Math.floor(params.durationSec / (60 / config.bpm));
    ...
    const leadNoteCount = Math.floor(params.durationSec / (60 / config.bpm / 2));
    ...
    if (params.lyrics) {
      ...
      await generateVocals({
        ...
        speed: 1.0,
        robot: false,
        language: params.vocalLanguages?.[0] || 'en'
      }, vocalsPath);
    }
    ...
    const audioAsset = await saveAsset('audio', 'audio/wav', audioData, params.durationSec);
    ...
    if (params.generateVideo) {
      ...
      videoAsset = await saveAsset('video', 'video/mp4', videoData, params.durationSec);
    }
